...
class MockDrone:
    ...
    def takeoff(self):
        if not self.is_flying:
            print("\n")
            print("TAKEOFF - Drone is now FLYING!")
            print("\n")
            self.is_flying = True
            self.position["y"] = 100
            return True
...
class RoboticHandHUD:
    ...
    def _draw_single_hand(self, frame, landmarks, x_start, y_start):
        ...
        if hand_width == 0 or hand_height == 0: return
...
class GestureDroneController:
    def __init__(self):
        print("Initializing Gesture Drone Controller")
    ...
    def switch_mode(self):
        ...
        if self.mode == "STATIC":
            ...
            print("Use fingers for precise control.")
    ...
    def handle_two_hand_gestures(self, hands_data):
        ...
        return action
    def handle_static_gesture(self, gesture_name, hand_orientation=None):
        if not self.is_flying:
            return
        if self.mode != "STATIC":
            return
    ...
    def handle_dynamic_gesture(self, frame):
        ...
        if self.dynamic_detector.detect_wave():
            ...
            return "WAVE"
        return None
    def draw_ui(self, frame, gesture, safety_action, dynamic_gesture, num_hands):
        h, w = frame.shape[:2]
        cv2.rectangle(frame, (10, 10), (350, 160), (0, 0, 0), -1)
        cv2.rectangle(frame, (10, 10), (350, 160), (0, 255, 0), 1)
        ...
        cv2.putText(frame, f"MODE: {self.mode}", (20, 75), cv2.FONT_HERSHEY_SIMPLEX, 0.7, mode_color, 2)
        if dynamic_gesture:
            cv2.putText(frame, f"CMD: {dynamic_gesture}", (20, 110), cv2.FONT_HERSHEY_SIMPLEX, 0.7, Config.COLOR_ORANGE, 2)
        elif safety_action:
             cv2.putText(frame, f"CMD: {safety_action}", (20, 110), cv2.FONT_HERSHEY_SIMPLEX, 0.7, Config.COLOR_RED, 2)
        else:
            cv2.putText(frame, f"CMD: {gesture}", (20, 110), cv2.FONT_HERSHEY_SIMPLEX, 0.7, Config.COLOR_YELLOW, 2)
        if self.mode == "DYNAMIC":
            cv2.putText(frame, "DYNAMIC ACTIVE", (w//2 - 100, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.0, Config.COLOR_ORANGE, 2)
        if self.drone.follow_mode:
            cv2.putText(frame, "FOLLOW MODE", (w//2 - 100, h - 50),
                       cv2.FONT_HERSHEY_SIMPLEX, 1.0, Config.COLOR_GREEN, 3)
        cv2.putText(frame, f"FPS: {self.fps:.1f}", (w - 120, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 1)
        cv2.putText(frame, "'q'=Quit | 'd'=Toggle Mode | Rock(🤘)=Switch", (20, h - 15), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200, 200, 200), 1)
        return frame
    def run(self):
        ...
        while True:
            ...
            if results.multi_hand_landmarks:
                ...
                if num_hands >= 2:
                    ...
                    if self.mode == "STATIC":
                        gesture_text, conf = self.gesture_classifier.classify_gesture(hand)
                        self.handle_static_gesture(gesture_text)
                    else:
                        gesture_text = "HOVER (Dynamic)"
                        dynamic_text = self.handle_dynamic_gesture(frame)
            frame = self.draw_ui(frame, gesture_text, safety_text, dynamic_text, num_hands)
            ...
            cv2.imshow("Gesture Drone Controller - Final", frame)
            self.frame_count += 1
